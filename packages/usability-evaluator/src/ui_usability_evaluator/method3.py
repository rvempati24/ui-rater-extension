...
def main(argv: list[str] | None = None) -> int:
    ...
    immutable_names = [
        name for name in ("evidence", "website", "derived", "contract")
        if (case_dir / name).is_dir()
    ]
    before = {name: tree_digest(case_dir / name) for name in immutable_names}
    ...
    try:
        ...
        error = str(run_error)
    after = {name: tree_digest(case_dir / name) for name in immutable_names}
    if before != after:
